...
# 处理玩家输入和AI响应
def handle_chat(user_ids,user_inputs):
    # 将用户输入添加到历史记录中并显示
    # 遍历user_ids和user_inputs，将它们的元素配对后添加到历史记录中
    for user_id, user_input in zip(user_ids, user_inputs):
        st.session_state['stream_chat_history'].append({'label': user_id, 'message': user_input})
    display_history()
...
def main() -> None:
    ...
    for _ in range(1, MAX_GAME_ROUND + 1):
        ...
        with msghub(wolves, announcement=hint) as hub:
            ...
            # 广播狼人玩家 要杀死的角色
            messagetowolf=HostMsg(content=Prompts.to_wolves_res.format(dead_player[0]))
            hub.broadcast(
                messagetowolf,
            )
            print("广播给狼人玩家的语句",messagetowolf)
